...
namespace DxSprite {
	...
	//引数 1                                    要素数
	//戻値                                      なし
	SpriteDrawing3::SpriteDrawing3(int x, float tile) :
			num					(x),
			tilePixel     		(tile) {
		rec.reserve(num);
		cen.reserve(num);
	...
	//引数 2                                    要素数
	//戻値                                      なし
	SpriteDrawing3::SpriteDrawing3(LPCWSTR tex, int x, float tile) :
			num					(x),
			tilePixel			(tile) {
		rec.reserve(num);
		cen.reserve(num);
	...
	//引数 1                                    画像パス
	//戻値                                      HRESULT
	HRESULT SpriteDrawing3::setTexture(LPCWSTR path) {
	...
	//引数 1                                    テクスチャのポインタ
	//戻値                                      なし
	void SpriteDrawing3::setTexture(LPDIRECT3DTEXTURE9 pTex) {
		tex = pTex;
		// テクスチャサイズの取得
		D3DSURFACE_DESC desc;
	...
	//引数 5                                    RECT.top
	//戻値                                      なし
	void SpriteDrawing3::setRect(int sub, LONG right, LONG bottom, LONG left, LONG top) {
		rec[sub].top =  top;
	...
	//戻値                                      なし
	void SpriteDrawing3::setRectC(int sub, int start, int end) {
	...
	//引数 4                                    Center.z
	//戻値                                      なし
	void SpriteDrawing3::setCenter(int sub, float x, float y, float z) {
		cen[sub].x = x;
		cen[sub].y = y;
		cen[sub].z = z;
	}
	...
	//引数 3                                    Position.z
	//戻値                                      なし
	void SpriteDrawing3::setPosition(int sub, float x, float y, float z) {
		pos[sub].x = x;
		pos[sub].y = y;
		pos[sub].z = z;
	}
	...
	//引数 2                                    添え字
	//戻値                                      なし
	void SpriteDrawing3::setColor(D3DCOLOR c, int sub) {
		color[sub] = c;
	}
	...
	//引数 2                                    添え字
	//戻値                                      計算後の描画位置のX座標
	float SpriteDrawing3::addPositionX(float add, int sub) {
		pos[sub].x += add;
		return pos[sub].x;
	}
	...
	//引数 2                                    添え字
	//戻値                                      計算後の描画位置のX座標
	float SpriteDrawing3::addPositionY(float add, int sub) {
		pos[sub].y += add;
		return pos[sub].y;
	}
	...
	bool SpriteDrawing3::addAlpha(int alpha, int sub) {
	...
	//引数 1                                    添え字
	//戻値                                      なし
	void SpriteDrawing3::disableDraw(int sub) {
		draw[sub] = false;
	}
	...
	//引数 1                                    添え字
	//戻値                                      なし
	void SpriteDrawing3::enableDraw(int sub) {
		draw[sub] = true;
	}
	...
	//引数 1                                    チップ幅
	//戻値                                      なし
	void SpriteDrawing3::setTile(float t) {
		tilePixel = t;
	}
